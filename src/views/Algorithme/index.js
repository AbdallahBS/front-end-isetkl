...
import { getAlgorithme } from "actions";
import  { useEffect, useState } from "react";
import FileDownload from "js-file-download";
import { useDispatch, useSelector } from 'react-redux';
// core components
import ExamplesNavbar from "components/Navbars/ExamplesNavbar.js";
import ProfilePageHeader from "components/Headers/Algorithme.js";
import DemoFooter from "components/Footers/DemoFooter.js";
import { getFile } from "actions/File.action";
import { getAllAlgorithmeFiles } from "actions";
import { getStatisticsFile } from "actions";
import { getAlgorithmeFile } from "actions";
...
function ProfilePage() {
  ...
  useEffect(()=>{
    ...
    dispatch(getAllAlgorithmeFiles())
    
  },[]);

  const Algorithme = useSelector(state => state.Algorithme);
  const etat = useSelector(state => state.Algorithme);
  const Files = useSelector(state => state.File_Algorithme);
 const download =(name)=>{
 
 const form  = new FormData();
 form.append('name',name);
 dispatch(getAlgorithmeFile(form));
}
 if(etat.etat){
  ...
  if(Algorithme.Algorithme && Files.Files){
      ...
      <div className="section profile-content">
        <Container>
          <div className="owner">
            ...
            <div className="name">
              <h4 className="title">
               {Algorithme.Algorithme?Algorithme.Algorithme.algorithme[0].name:null}<br />
              </h4>
